-- ServerData for DCS World https://github.com/szporwolik/ServerData -> DCS Hook config component
ServerDataConfig = ServerDataConfig or {}
-- 变量初始化(Variable init)
ServerDataConfig.Version = "2.0"
-- 连接(Connection)
-- (int) [default: 3600] Base refresh rate in seconds to send status update
-- (int) [默认值: 3600] 发送状态更新的基本刷新率(秒)
ServerDataConfig.RefreshStatus = 3600
...
ServerDataConfig.MissionStartNoDeathWindow = 300
-- 本地化(Localisation)
-- (string) Message send to players connecting the server - Line 1
ServerDataConfig.MOTD_L1 = "Welcome to our server ! "
-- ServerData 与 DCS World集成统计和事件数据
-- (string) Message send to players connecting the server - Line 2
-- (string) 消息发送给连接服务器的玩家
ServerDataConfig.MOTD_L2 = "Stats and event data integrated with ServerData for DCS World"
-- (string) Information to send to players when ServerData connection is broken
-- (string) 从服务器数据连接中断时发送给玩家的信息
ServerDataConfig.ConnectionError_L1 = "ERROR: Connection broken - contact server admin!"
-- Debug
ServerDataConfig.DebugMode = 1
