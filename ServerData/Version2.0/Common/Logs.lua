...
ServerData.GetMulticrewCrew = function(owner_playerID)
  ...
  if _master_type == "F-14B" or _master_type == "Yak-52" or _master_type == "L-39C" or _master_type == "SA342M" or _master_type == "SA342Minigun" or _master_type == "SA342Mistral" or _master_type == "SA342L" or _master_type == "F-14A-135-GR" or _master_type == "C-101EB" or _master_type == "UH-1H" or _master_type == "C-101CC" then
  ...
  Debugger.AddLog("Multicrew check completed: " .. net.lua2json(_crew), 2)
  return _crew
end
...
ServerData.LogChat = function(playerID, msg, all)
  ...
  ServerData.send_udp_msg("LogChat", _TempData)
  Debugger.AddLog(_TempData, 1)
end
---事件日志
...
ServerData.LogEvent = function(log_type, log_content, log_arg_1, log_arg_2)
  ...
  Debugger.AddLog("event: " .. log_type .. ", arg1:" .. log_arg_1 .. ", arg2:" .. log_arg_2 .. ", content: " .. log_content, 1)
  ServerData.send_udp_msg("LogEvent", _TempData)
end
---记录单个玩家数据
---@param playerID any --玩家ID
ServerData.LogStats = function(playerID)
  ...
  Debugger.AddLog("Sending stats data", 1)
  ServerData.send_udp_msg("LogStats", _TempData)
end
---记录所有玩家数据
...
ServerData.LogLogin = function(playerID)
  ...
  Debugger.AddLog("Sending login event", 1)
  ServerData.send_udp_msg("LogLogin", _TempData)
end
...
ServerData.LogStatsCount = function(argPlayerID, argAction, argTimer)
  ...
  Debugger.AddLog("Stats data prepared", 2)
...
ServerData.LogStatsGet = function(playerID)
  ...
  Debugger.AddLog("Getting stats data finished; took: " .. _delay .. "us", 2)
...
ServerData.UpdateStatus = function()
  ...
  -- Send
  local _delay = (DCS.getRealTime() - _now) * 1000000
  Debugger.AddLog("Updated status data; sending (" .. _delay .. "us)", 2)
  ServerData.send_udp_msg("UpdateStatus", ServerData.StatusData)
end
---更新并发送插槽数据
ServerData.UpdateSlots = function()
  ...
  local _delay = (DCS.getRealTime() - _now) * 1000000
  Debugger.AddLog("Updated slots data; sending (" .. _delay .. "us)", 2)
  ServerData.send_udp_msg("UpdateSlots", ServerData.SlotsData)
end
---任务信息更新的主要功能
ServerData.UpdateMission = function()
  ...
  local _delay = (DCS.getRealTime() - _now) * 1000000
  Debugger.AddLog("Updated mission data; sending (" .. _delay .. "us)", 2)
  ServerData.send_udp_msg("UpdateMission", ServerData.MissionData)
end
