...
SourceObj.updateSourcePointsByEvent = function(_unit, _ucid, _event)
  ...
  if _event == "takeoff" then
    ...
    if SourceObj.playerSource[_ucid]["point"] - sourcePointChange > 0 then
      ...
      SourceObj.SaveSourcePoint()
      local text = string.format("起飞成功,本次消耗:%d点,剩余:%d点.\n详细信息:%s", tostring(sourcePointChange), tostring(SourceObj.playerSource[_ucid]["point"]), tostring(countInfo))
      trigger.action.outTextForGroup(_groupId, text, 20, true)
    else
      local text = string.format("你的资源点剩余(%d),挂载需要:%d,马上就爆炸了~BOOM", SourceObj.playerSource[_ucid]["point"], sourcePointChange)
  ...
  elseif _event == "landing" then
    ...
    SourceObj.playerSource[_ucid]["point"] = SourceObj.playerSource[_ucid]["point"] + sourcePointChange
    SourceObj.SaveSourcePoint()
    local text = string.format("降落成功,返还:%d点,剩余:%d点.\n详细信息:%s", tostring(sourcePointChange), tostring(SourceObj.playerSource[_ucid]["point"]), tostring(countInfo))
...
SourceObj.eventAddPoint = function(_event, _point, _ucid, _groupId)
  ...
  local text = ""
  if _point > 0 then
    text = string.format("%s,奖励:%s点,你的资源点剩余:%d点", _event, tostring(_point), tostring(SourceObj.playerSource[_ucid]["point"]))
  else
    text = string.format("%s,惩罚:%s点,你的资源点剩余:%d点", _event, tostring(_point), tostring(SourceObj.playerSource[_ucid]["point"]))
  end
  trigger.action.outTextForGroup(_groupId, text, 10)
end
...
SourceObj.addF10SourceMenu = function(groupId)
  if not SourceObj.addedF10Menu[groupId] then
    local status, err =
      pcall(
      function()
        local _rootPath = missionCommands.addSubMenuForGroup(groupId, "资源系统")
        missionCommands.addCommandForGroup(groupId, "查询资源点", _rootPath, SourceObj.getPointByGroupID, groupId)
      end
    )
    if (not status) then
...
SourceObj.getPointByGroupID = function(groupId)
  if groupId ~= nil then
    ...
    if _ucid then
      trigger.action.outTextForGroup(groupId, string.format("你当前拥有资源点:%s", tostring(SourceObj.playerSource[_ucid]["point"])), 30)
    end
  end
end
